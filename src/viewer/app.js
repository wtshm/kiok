...
function renderPager(offset, count, onPrev, onNext) {
  ...
  if (hasPrev)
    html += `<button class="pager-btn" id="pg-prev">\u2190 Prev</button>`;
  html += `<span class="pager-info">Page ${page}</span>`;
  if (hasNext)
    html += `<button class="pager-btn" id="pg-next">Next \u2192</button>`;
  html += "</div>";
  return {
    html,
    bind() {
...
async function loadSessions(offset) {
  ...
  let html =
    '<div class="grid-header"><span>Project</span><span>Scope</span><span>Chunks</span><span>Session ID</span><span style="text-align:right">Imported</span></div>';
...
async function doSearch(q) {
  ...
  const r = await fetch(
    "/api/search?q=" + encodeURIComponent(q) + "&limit=" + PAGE_SIZE,
  );
  const rows = await r.json();
  if (!rows.length) {
    $("#content").innerHTML =
